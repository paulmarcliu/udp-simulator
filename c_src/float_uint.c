...
#define S_FUNCTION_NAME float_uint
/*<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<*/
...
#define INPUT_0_DTYPE       SS_SINGLE
...
static uint32_T ExtractFloat(float Val)
{
...
/* 
 * Function: mdlOutputs =======================================================
 - Runs every simulation event
 *
*/
static void mdlOutputs(SimStruct *S, int_T tid)
{
    const float *u = (const float *)ssGetInputPortSignal(S,0);
    uint32_T       *y = (uint32_T *)ssGetOutputPortSignal(S,0);

    y[0]=ExtractFloat(u[0]);
}
